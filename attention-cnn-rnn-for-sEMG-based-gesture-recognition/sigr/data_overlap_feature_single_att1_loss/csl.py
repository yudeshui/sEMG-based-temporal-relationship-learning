...
def get_intra_session_val(fold, batch_size, preprocess, balance_gesture=True, 
                           window=1, rnn=False, dense_window=True, sub_window_length=1, sub_window_step=1,**kargs):
    # print '%%%%%%%%%%%%%%%%%%%%%%', preprocess
...
def get_universal_intra_session_data(fold, batch_size, preprocess, balance_gesture, 
                                     window, rnn, dense_window, rnn_window, sub_window_length, sub_window_step, signal,dwpt, **kargs):
    get_trial = GetTrial(preprocess=preprocess)
    load = partial(get_data,
                   amplitude_weighting=kargs.get('amplitude_weighting', False),
                   amplitude_weighting_sort=kargs.get('amplitude_weighting_sort', False),
...
def get_intra_session_data(fold, batch_size, preprocess, balance_gesture, 
                           window, rnn, dense_window, rnn_window, sub_window_length, sub_window_step, signal, dwpt, **kargs):
    get_trial = GetTrial(preprocess=preprocess)
    load = partial(get_data,
                   amplitude_weighting=kargs.get('amplitude_weighting', False),
