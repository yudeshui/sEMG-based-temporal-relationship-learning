...
from . import utils
from . import module_overlap_feature_single_att1_loss as module
...
def _crossval_predict_aux(self, Mod, get_crossval_val, fold, context, dataset_args=None):
    ...
    tmp = mod.predict(utils.LazyProxy(Val))
    return tmp
...
def _crossval_predict(self, **kargs):
    ...
    get_crossval_val = kargs.pop('get_crossval_val')
    dataset_args=kargs.pop('dataset_args')
    ...
    if proba:
        return _crossval_predict_proba_aux(self, Mod=Mod, get_crossval_val=get_crossval_val, fold=fold, context=context, dataset_args=dataset_args)
    else:
        return _crossval_predict_aux(self, Mod=Mod, get_crossval_val=get_crossval_val, fold=fold, context=context, dataset_args=dataset_args)
