...
class Symbol(object):
    ...
    def get_feature(
        ...
        if num_pixel:
            for i in range(num_pixel):
                print 'self.pixel_reduce_filter[i]:', i, self.num_pixel_reduce_filter[i]
    ...
    def __init__(
        ...
        rnn=False,
        rnn_type = 'lstm',
        num_rnn_hidden=constant.NUM_RNN_HIDDEN,
        num_rnn_layer=constant.NUM_RNN_LAYER,
        rnn_last=0,
        rnn_dropout=constant.RNN_DROPOUT,
        rnn_shortcut=False,
        rnn_bn=True,
        rnn_window=None,
        rnn_grad_scale=True,
        ...
        **kargs
        ...
        self.genimage = genimage
        print "symbol.num_semg_row:", self.num_semg_row
        print "symbol.num_semg_col:", self.num_semg_col
        print "self.num_channel:", self.num_channel
        print "genimage:", self.genimage


        def get_stream(prefix):
            if prefix:
                net1 = mx.symbol.Variable(name=prefix + 'data', attr={'tag': '1'})

            else:
                net1 = mx.symbol.Variable(name=prefix + 'data')


            if self.rnn:
                assert not zscore_bng
                # if self.num_channel > 1:
                net1 = mx.symbol.Reshape(net1, shape=(0, -1, self.num_channel, self.num_semg_row, self.num_semg_col))
                net1 = mx.symbol.SwapAxis(net1, dim1=0, dim2=1)

                net1 = mx.symbol.Reshape(net1, shape=(-1, self.num_channel, self.num_semg_row, self.num_semg_col))


            if self.genimage ==1:
                net1 = mx.symbol.Reshape(net1, shape=(-1,1, self.num_channel * self.num_semg_row, self.num_semg_col))
            elif self.genimage==2:
                net1 = mx.symbol.Reshape(net1, shape=(-1,1,1, self.num_channel * self.num_semg_row * self.num_semg_col))



            if zscore:
                net1 = (self.get_bng if zscore_bng else self.get_bn)(prefix + 'zscore_1', net1)
                shortcut = net1

            features1 = self.get_feature(
                data=net1,
                num_filter=num_filter,
                num_pixel=num_pixel,
            ...
            features1 = features1[prefix + 'bottleneck']
        ...
        if self.rnn:
            print 'rnn:', self.rnn
            feature = mx.symbol.Reshape(feature, shape=(self.rnn_window, -1, num_bottleneck))
            # feature = mx.symbol.Reshape(feature, shape=(self.rnn_window, -1, 2*num_bottleneck))

            feature = mx.symbol.SwapAxis(feature, dim1=0, dim2=1)

            if rnn_shortcut:
                shortcut = features['shortcut']
                shortcut = mx.symbol.Reshape(shortcut, shape=(self.rnn_window, -1, constant.NUM_SEMG_POINT))
            ...
            from .mru import mru_unroll
            from .bilstm import bilstm_unroll
            from .pbilstm import pbilstm_unroll
            from .rlstm import rlstm_unroll
            from .rlstmc import rlstmc_unroll
            from .rlstmh import rlstmh_unroll
            from .rgru import rgru_unroll
            from .rmgu import rmgu_unroll
            from .convlstm import convlstm_unroll
            from .lclstm import lclstm_unroll
            ...
            if rnn_type == 'lstm':
                feature = lstm_unroll(
                    ...
                    seq_len=self.rnn_window,
                    num_hidden=num_rnn_hidden,
                    dropout=rnn_dropout,
            ...
            elif rnn_type == 'pbilstm':
                feature = pbilstm_unroll(
                    prefix='',
                    data=feature,
                    num_rnn_layer=num_rnn_layer,
                    seq_len=self.rnn_window,
            ...
            elif rnn_type == 'rlstmc':
                feature = rlstmc_unroll(
                    prefix='',
                    data=feature,
                    num_rnn_layer=num_rnn_layer,
                    seq_len=self.rnn_window,
            ...
            elif rnn_type == 'rlstmh':
                feature = rlstmh_unroll(
                    prefix='',
                    data=feature,
                    num_rnn_layer=num_rnn_layer,
                    seq_len=self.rnn_window,
            ...
            elif rnn_type == 'lclstm':
                feature = lclstm_unroll(
                    prefix='',
                    data=feature,
                    num_rnn_layer=num_rnn_layer,
                    seq_len=self.rnn_window,
            ...
            feature1=feature
            concat_feature = mx.sym.Reshape(data=mx.sym.Concat(*feature, dim=1), shape=(-1,self.rnn_window, num_rnn_hidden))
            M = []
            a = []
            r = []
            impt=[]
            weight_v = mx.sym.Variable('atten_v_bias', shape=(num_rnn_hidden, 1))
            weight_u = mx.sym.Variable('atten_u_weight', shape=(num_rnn_hidden, num_rnn_hidden))            
            for i in range(rnn_window):


                # M.append(self.get_fc_tanh(name='atten_%d'%i, data=feature1[i], num_hidden=num_rnn_hidden, no_bias=False))
                # M.append(self.get_tanh(name='atten_%d'%i, data=feature1[i]))
                # M.append(self.get_fc_bn_tanh(name='atten_%d'%i, data=feature1[i], num_hidden=num_rnn_hidden, no_bias=True))
                # M.append(self.get_fc_tanh_drop(name='atten_%d'%i, data=feature1[i], num_hidden=num_rnn_hidden, no_bias=False))
                # tmp = self.get_fc_tanh(name='atten_%d'%i, data=feature1[i], num_hidden=num_rnn_hidden, no_bias=False) 

                tmp = mx.sym.dot(feature1[i], weight_u, name='atten_u_%d'%i)
                tmp = mx.sym.Activation(tmp,act_type='tanh')
                # tmp = self.get_fc_bn_relu_drop(name='atten_%d'%i, data=feature1[i], num_hidden=num_rnn_hidden)
                # impt.append(mx.sym.SoftmaxActivation(name='atten_softmax_imp_%d'%i, data=tmp))
                tmp = mx.sym.dot(tmp, weight_v, name='atten_v_%d'%i)
                M.append(tmp)
            
            # impt = mx.sym.Reshape(data=mx.sym.Concat(*impt, dim=1), shape=(-1,self.rnn_window, num_rnn_hidden))
            M = mx.sym.Concat(*M, dim=1)
            a = mx.symbol.SoftmaxActivation(name='atten_softmax_%d'%i, data=M)
            a = mx.sym.Reshape(data=a, shape=(-1,self.rnn_window, 1)) 
            r = mx.symbol.broadcast_mul(name='atten_r_%d'%i, lhs=a, rhs=concat_feature)
            # r = mx.symbol.broadcast_mul(name='atten_r_imp_%d'%i, lhs=concat_feature, rhs=impt)
        ...
        if self.for_training:
            ...
            if self.rnn and (rnn_last == 0 or rnn_last > 1):
                gesture_label = mx.symbol.Variable(name='att_gesture_softmax_label') # m
                gesture_label = mx.symbol.Reshape(mx.symbol.Concat(*[
                    mx.symbol.Reshape(gesture_label, shape=(0, 1))
                    for i in range(rnn_window)], dim=0), shape=(-1,))
                gesture_branch_kargs['label'] = gesture_label
                if rnn_grad_scale:
                    gesture_branch_kargs['grad_scale'] = 1 / (rnn_last or self.rnn_window)
                    # gesture_branch_kargs['grad_scale'] = 0.001 / (rnn_last or self.rnn_window)
                    # gesture_branch_kargs['grad_scale'] = 10 / (rnn_last or self.rnn_window)

                    # gesture_branch_kargs['grad_scale'] = 0
        ...
        if self.rnn and (rnn_last == 0 or rnn_last > 1):
            ...
            att_gesture_label = mx.symbol.Reshape(mx.symbol.Concat(*[
                mx.symbol.Reshape(att_gesture_label, shape=(0, 1))
                for i in range(1)], dim=0), shape=(-1,))
            att_gesture_branch_kargs['label'] = att_gesture_label
            if rnn_grad_scale:
                # att_gesture_branch_kargs['grad_scale'] = 1 / (rnn_last or self.rnn_window)
                att_gesture_branch_kargs['grad_scale'] = 0.1 / (rnn_last or self.rnn_window)

        att_gesture_softmax, att_gesture_fc = self.get_branch(
            name='gesture', # m
            data=z,
            num_class=num_gesture,
            num_block=num_gesture_block,
        ...
        if self.rnn:
            self.net.data_shape_1 = self.num_channel * self.rnn_window
        else:
            self.net.data_shape_1 = self.num_channel

        if self.rnn:
            self.net.num_feature = num_rnn_hidden
        else:
        ...
        if self.rnn:
            ...
            net1 = mx.symbol.Variable(name='data')
            ...
            # self.net.bottleneck = feature1+feature2
            self.net.bottleneck = feature1
            ...
            print 'rnn_last:', rnn_last

            concat_net = mx.sym.Reshape(data=mx.sym.Concat(*net, dim=1), shape=(-1,self.rnn_window, num_rnn_hidden))
            M = []
            impt = []
            weight_v = mx.sym.Variable('atten_v_bias', shape=(num_rnn_hidden, 1))
            weight_u = mx.sym.Variable('atten_u_weight', shape=(num_rnn_hidden, num_rnn_hidden))
            for i in range(rnn_window):
                ...
                # M.append(self.get_fc_bn_tanh(name='atten_%d'%i, data=feature1[i], num_hidden=num_rnn_hidden, no_bias=True))
                # M.append(self.get_fc_tanh_drop(name='atten_%d'%i, data=feature1[i], num_hidden=num_rnn_hidden, no_bias=False))
                # tmp = self.get_fc_tanh(name='atten_%d'%i, data=net[i], num_hidden=num_rnn_hidden, no_bias=False)  
                tmp = mx.sym.dot(net[i], weight_u, name='atten_u_%d'%i)
                tmp = mx.sym.Activation(tmp,act_type='tanh')
                # tmp = self.get_fc_bn_relu_drop(name='atten_%d'%i, data=net[i], num_hidden=num_rnn_hidden)
                # impt.append(mx.sym.SoftmaxActivation(name='atten_softmax_imp_%d'%i, data=tmp))
                tmp = mx.sym.dot(tmp, weight_v, name='atten_v_%d'%i)
                M.append(tmp)
            # impt = mx.sym.Reshape(data=mx.sym.Concat(*impt, dim=1), shape=(-1,self.rnn_window, num_rnn_hidden))
            ...
            a = mx.symbol.SoftmaxActivation(name='atten_softmax_%d'%i, data=M)
            a = mx.sym.Reshape(data=a, shape=(-1,self.rnn_window, 1)) 
            r = mx.symbol.broadcast_mul(name='atten_r_%d'%i, lhs=a, rhs=concat_net)
            # r = mx.symbol.broadcast_mul(name='atten_r_imp_%d'%i, lhs=concat_net, rhs=impt)
            z = mx.sym.sum(data=r, axis=1)
            ...
            # elif rnn_last < 0:
            #     net = mx.sym.Pooling(
            #         mx.sym.Concat(*[mx.sym.Reshape(h, shape=(0, 0, 1, 1)) for h in net[rnn_last:]], dim=3),
            #         kernel=(1, 1),
            #         global_pool=True,
            #         pool_type='avg'
            #     )
            # elif rnn_last:
            #     net = mx.sym.Concat(*net[-rnn_last:], dim=0)
            # else:
            ...
            # if rnn_last == 0 or rnn_last > 1:
            #     net = mx.sym.Reshape(net, shape=((rnn_last or self.rnn_window), -1, num_gesture))
            #     net = mx.symbol.sum(net, axis=0) / (rnn_last or self.rnn_window)
            #     net = mx.sym.Reshape(net, shape=(-1, num_gesture))
            self.net.rnn = net
            #  self.net.rnn = self.get_branch(
                #  name='gesture',
                #  data=net,
                #  num_class=num_gesture,
                #  num_block=num_gesture_block,
            ...
            self.net.rnn_window = self.rnn_window
            self.net.num_rnn_hidden = num_rnn_hidden
            self.net.num_rnn_layer = num_rnn_layer
            self.net.rnn_bn = self.rnn_bn
