...
import time
import os
from .symbol_overlap_feature_single_att1_loss import get_symbol
...
class SymbolMixin(object):
    ...
    def __init__(self, **kargs):
        ...
        # arg_shape, out_shape, aux_shape = symbol.infer_shape(data=(1,20,1,10))
        # print symbol.list_arguments(), arg_shape
...
class FaugMixin(object):
    ...
    def forward_backward(self, data_batch):
        # print 'faug:', self.faug, self.faug_classwise
        ...
        # print 'data type:', data_batch.data
...
class RNNBindMixin(object):

    def __init__(self, **kargs):
        parent = kargs.get('parent', {})
        self.rnn = kargs.pop('rnn', getattr(parent, 'rnn', False))
        self.rnn_type = kargs.pop('rnn_type', getattr(parent, 'rnn_type', 'lstm'))
        self.num_rnn_hidden = kargs.pop('num_rnn_hidden', getattr(parent, 'num_rnn_hidden', constant.NUM_RNN_HIDDEN))
        self.num_rnn_layer = kargs.pop('num_rnn_layer', getattr(parent, 'num_rnn_layer', constant.NUM_RNN_LAYER))
        symbol_kargs = kargs.pop('symbol_kargs', getattr(parent, 'symbol_args', {})).copy()
        symbol_kargs.update(
            rnn=self.rnn,
            num_rnn_hidden=self.num_rnn_hidden,
            num_rnn_layer=self.num_rnn_layer
        )
        kargs['symbol_kargs'] = symbol_kargs
        super(RNNBindMixin, self).__init__(**kargs)
...
class RNNMixin(RNNBindMixin):
    ...
    def score(
        ...
        # return super(RNNMixin, self).score(
        #         eval_data,
        #         eval_metric,
        #         num_batch,
        #         batch_end_callback,
        ...
        mod = self._get_rnn_mod(bottleneck)
        return mod.score(
            bottleneck,
            eval_metric,
            num_batch,
    ...
    def predict(
        ...
        # return super(RNNMixin, self).predict(
        #         eval_data,
        #         num_batch,
        #         merge_batches,
        #         reset,
    ...
    def _get_bottleneck(self, data, **kargs):
        ...
        return MovingWindow(
            ...
            window=self.symbol.rnn_window,
            segment=data.segment,
            label=data.gesture,
            stride = int(data.sub_window_step/data.sub_window_length)
            # stride=1 if not data.dense_window else data.window
        )

    def _get_rnn_mod(self, data):
        if self.symbol.rnn_bn:
            class Mod(RNNBindMixin, AdaBNModule):
                pass
            if data.stride > 1:
                Mod = partial(Mod, num_adabn_epoch=1)
        else:
            class Mod(RNNBindMixin, Base):
                pass
        mod = Mod(
            context=self._context,
            symbol=self.symbol.rnn,
...
class ExecutorGroup(mx.module.executor_group.DataParallelExecutorGroup):
    ...
    def __init__(self, *args, **kargs):
        self.num_rnn_layer = kargs.pop('num_rnn_layer')
        self.num_rnn_hidden = kargs.pop('num_rnn_hidden')
        self.rnn_type = kargs.pop('rnn_type')
    ...
    def _bind_ith_exec(self, i, data_shapes, label_shapes, shared_group):
        ...
        print 'module bind rnn type:', self.rnn_type
        if self.rnn_type in ['lstm', 'rlstm', 'rlstmc', 'rlstmh']:
            shapes.update({'rnn_l%d_init_c' % i: (batch_size, self.num_rnn_hidden)
                       for i in range(self.num_rnn_layer)})
            shapes.update({'rnn_l%d_init_h' % i: (batch_size, self.num_rnn_hidden)
                       for i in range(self.num_rnn_layer)})
        elif self.rnn_type in ['gru', 'mru', 'rgru', 'rmgu']:
            shapes.update({'rnn_l%d_init_h' % i: (batch_size, self.num_rnn_hidden)
                       for i in range(self.num_rnn_layer)})
        elif self.rnn_type in ['bilstm', 'pbilstm']:
            shapes.update({'rnn_l0_init_c' : (batch_size, self.num_rnn_hidden)})
            shapes.update({'rnn_l0_init_h' : (batch_size, self.num_rnn_hidden)})
            shapes.update({'rnn_l1_init_c' : (batch_size, self.num_rnn_hidden)})
            shapes.update({'rnn_l1_init_h' : (batch_size, self.num_rnn_hidden)}) 
        elif self.rnn_type in ['convlstm', 'lclstm']:
            shapes.update({'rnn_l%d_init_c' % i: (batch_size, self.num_rnn_hidden,1,128)
                       for i in range(self.num_rnn_layer)})   #num_bottleneck
            shapes.update({'rnn_l%d_init_h' % i: (batch_size, self.num_rnn_hidden,1,128)
                       for i in range(self.num_rnn_layer)})   
        else:          
            print 'wrong rnn type:', self.rnn_type
        ...
        if self.input_types is None:
            ...
            input_types = self.input_types
        types = {}
        if self.rnn_type in ['lstm', 'rlstm', 'rlstmc', 'rlstmh', 'convlstm', 'lclstm']:
            types.update({'rnn_l%d_init_c' % i: mx.base.mx_real_t
                      for i in range(self.num_rnn_layer)})
            types.update({'rnn_l%d_init_h' % i: mx.base.mx_real_t
                      for i in range(self.num_rnn_layer)})
        elif self.rnn_type in ['gru', 'mru', 'rgru', 'rmgu']:
            types.update({'rnn_l%d_init_h' % i: mx.base.mx_real_t
                      for i in range(self.num_rnn_layer)})
        elif self.rnn_type in ['bilstm', 'pbilstm']:
            types.update({'rnn_l0_init_c': mx.base.mx_real_t})
            types.update({'rnn_l0_init_h': mx.base.mx_real_t})
            types.update({'rnn_l1_init_c': mx.base.mx_real_t})
            types.update({'rnn_l1_init_h': mx.base.mx_real_t})      
        else:
            print 'wrong rnn type:', self.rnn_type
...
class MovingWindow(mx.io.DataIter):
    ...
    def __init__(
        ...
        print 'moving window num_bottleneck:', self.num_bottleneck
        ...
        index = []
        cursor = -1
        segcount=[list(self.segment).count(i) for i in range(10)]
        print 'segcount:', segcount
        # print 'segment length:', self.segment.shape
        ...
        # print index
        cut = lambda a: np.hstack(a[begin:max(end - (self.window-1)*self.stride,0)]
        ...
        #         # print 's1:', self.segment[cursor:cursor + (self.window - 1) * self.stride:self.stride]
        #         # print 's2:', self.segment[cursor + self.stride:cursor + self.window * self.stride:self.stride]

        #         if np.all(self.segment[cursor:cursor + (self.window - 1) * self.stride:self.stride] == self.segment[cursor + self.stride:cursor + self.window * self.stride:self.stride]):
        #             # print 's1:', self.segment[cursor:cursor + (self.window - 1) * self.stride:self.stride]
        #             if cursor + (self.window - 1) * self.stride >= self.data.shape[0]:
        #                 break
        #             index.append(cursor)
        ...
        #     # index.append(cursor)
        # self.index = np.array(index)
        # # print 'index length', len(self.index[self.index>=991*39])
...
class Module(RNNMixin, AdaBNMixin, FaugMixin, Base):
    ...
    def __init__(self, **kargs):
        ...
        self.clip_gradient = kargs.pop('clip_gradient', 0.5)
        symbol_kargs = kargs.pop('symbol_kargs', {}).copy()
        symbol_kargs.update(
        ...
        kargs['symbol_kargs'] = symbol_kargs
        label_names = ['gesture_softmax_label','att_gesture_softmax_label']
        if self.soft_label:
...
class Sigropt(mx.optimizer.NAG):
    ...
    def norm_clipping(self, grad, threshold):
        # print 'threshold:', threshold
        # assert isinstance(params_grad, dict)
        # grad_array = params_grad.asnumpy()
        # print grad_array
        norm_val = mx.nd.norm(grad).asnumpy()
        # print norm_val
        # print('grad norm: %g' % norm_val)
        ratio = 1.0
        if norm_val[0] > threshold:
            ratio = threshold / norm_val[0]
            # print 'ratio:', ratio
            grad = grad * ratio
        return grad
    ...
    def update(self, index, weight, grad, state):
        ...
        if self.clip_gradient is not None:
            ...
            # print grad.asnumpy()
            grad = self.norm_clipping(grad, self.clip_gradient)
            # grad = self.norm_clipping(grad, 0.5)
...
def _predict(mod, val):
    ...
    # print utils.continuous_segments(segment)
    # print 'sub_window_step in predict:', val.sub_window_step
    if mod.rnn:
        # print "*************************************"
        cut = lambda a: np.hstack(a[begin:max(end - ((mod.rnn_window-1)*val.sub_window_step),0)]
                                  for begin, end in utils.continuous_segments(segment))
        true = cut(true)
        segment = cut(segment)
    print "true shape:", true.shape
