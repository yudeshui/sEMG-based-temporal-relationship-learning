...
from .module_overlap_feature_single_att1_loss import Module
from .data_overlap_feature_single_att1_loss import Preprocess, Dataset
...
@click.option('--window-step', type=int, help='Window step. Multi-frame as image channels')
@click.option('--sub-window-length', type=int, help='Sub-window length')
@click.option('--sub-window-step', type=int, help='Sub-window step')
@click.option('--rnn-window', type=int)
...
@click.option('--dwpt', is_flag=True, help='Deprecaded. Perform DWPT and use spectrum amplitude as image channels. Cannot be used on non-uniform (segment length) dataset like NinaPro')
@click.option('--dwpt-append', is_flag=True, help='Append DWPT feature to raw frames in channel axis')
@click.option('--signal', is_flag=True, help='')
@click.option('--dual-stream', is_flag=True, help='Use raw frames and DWPT feature as dual-stream')
@click.option('--zscore/--no-zscore', default=True, help='Use z-score normalization on input')
@click.option('--zscore-bng', is_flag=True, help='Use global BatchNorm as z-score normalization, for window > 1 or DWPT')
@click.option('--rnn', is_flag=True)
@click.option('--num-rnn-hidden', type=int, default=constant.NUM_RNN_HIDDEN, help='Kernels of the hidden layers in rnn')
@click.option('--num-rnn-layer', type=int, default=constant.NUM_RNN_LAYER, help='Number of the hidden layers in rnn')
@click.option('--dense-window/--no-dense-window', default=True, help='Dense sampling of windows during training')
@click.option('--rnn-last', type=int, default=0)
@click.option('--rnn-dropout', type=float, default=constant.RNN_DROPOUT, help='rnn dropout ratio')
@click.option('--rnn-shortcut', is_flag=True)
@click.option('--rnn-bn/--no-rnn-bn', default=True, help='BatchNorm in rnn')
@click.option('--rnn-grad-scale/--no-rnn-grad-scale', default=True, help='Grad scale by the number of rnn output')
...
@click.option('--clip-gradient',type=float, default=0.5, help='Threshold of gradient clipping')
@click.option('--genimage', type=int, default=0, help='type of images, 0 means (20,1,10), 1 means (1,20,10)')
@click.option('--dataset', type=click.Choice(['ninapro-db1-dwpt',
                                              'ninapro-db1-phiny',
                                              'ninapro-db1-all-signal-feature-lowpass-win-19-stride-1',
                                              'ninapro-db1-all-signal-feature-lowpass-win-14-stride-1'
                                              ]), required=True)
...
@click.option('--rnn-type', type=click.Choice(['lstm',
                                               'gru',
                                               'mru',
                                               'bilstm',
                                               'pbilstm',
...
def crossval(args):
    ...
    with Context(args.log, parallel=True):
        ...
        mod = Module.parse(
            ...
            rnn=args.rnn,
            rnn_type = args.rnn_type,
            num_rnn_hidden=args.num_rnn_hidden,
            ...
            symbol_kargs=dict(
                ...
                num_channel=train.num_channel // (args.rnn_window or 1),
                ...
                rnn=args.rnn,
                rnn_type=args.rnn_type,
                rnn_last=args.rnn_last,
                rnn_dropout=args.rnn_dropout,
                rnn_shortcut=args.rnn_shortcut,
                rnn_bn=args.rnn_bn,
                rnn_window=args.rnn_window,
                rnn_grad_scale=args.rnn_grad_scale,
